# -*- coding: utf-8 -*-

import scrapy

class RaceItem(scrapy.Item):
    racenumber = scrapy.Field()
    racedate = scrapy.Field()
    racegoing = scrapy.Field()
    raceclass= scrapy.Field()
    racedistance  = scrapy.Field()
    racecoursecode = scrapy.Field()
    raceindex = scrapy.Field()
    racingincidentreport = scrapy.Field()
    results_url = scrapy.Field()
    racingincidentreport = scrapy.Field()
    horsereport = scrapy.Field()
    horsenumber = scrapy.Field()
    horsename = scrapy.Field()
    horsecode = scrapy.Field()
    sec_timelist = scrapy.Field()
    horse_url =  scrapy.Field()
    horse_smartform_url=  scrapy.Field()
    marginsbehindleader=scrapy.Field()
    jockeyname = scrapy.Field()
    jockeycode = scrapy.Field()
    trainername = scrapy.Field()
    trainercode = scrapy.Field()
    actualwt = scrapy.Field()
    declarhorsewt = scrapy.Field()
    jockey2horsewt = scrapy.Field()
    draw = scrapy.Field()
    lbw = scrapy.Field()
    runningposition = scrapy.Field()
    finishtime = scrapy.Field()
    winodds = scrapy.Field()
    winoddsrank= scrapy.Field()
    earlypacepoints_this = scrapy.Field()
    runners_list= scrapy.Field()
    isScratched= scrapy.Field()
    placenum= scrapy.Field()
    place = scrapy.Field()

    sirename = scrapy.Field()
    ownername = scrapy.Field()
    damname = scrapy.Field()
    damsire = scrapy.Field()

    win_combo_div = scrapy.Field()
    place_combo_div = scrapy.Field()
    qn_combo_div = scrapy.Field()
    qnp_combo_div = scrapy.Field()
    tce_combo_div = scrapy.Field()
    trio_combo_div = scrapy.Field()
    f4_combo_div = scrapy.Field()
    qtt_combo_div = scrapy.Field()
    dbl9_combo_div = scrapy.Field()
    dbl8_combo_div = scrapy.Field()
    dbl7_combo_div = scrapy.Field()
    dbl6_combo_div = scrapy.Field()
    dbl5_combo_div = scrapy.Field()
    dbl4_combo_div = scrapy.Field()
    dbl3_combo_div = scrapy.Field()
    dbl2_combo_div = scrapy.Field()
    dbl1_combo_div = scrapy.Field()
    dbl10_combo_div = scrapy.Field()

    dbltrio1_combo_div = scrapy.Field()
    dbltrio2_combo_div = scrapy.Field()
    dbltrio3_combo_div = scrapy.Field()
    
    sixup_combo_div = scrapy.Field()
    jockeychallenge_combo_div = scrapy.Field()

    h_raceindexes = scrapy.Field()
    h_raceurls = scrapy.Field()
    h_places=scrapy.Field()
    h_racedates=scrapy.Field()
    h_goings=scrapy.Field()
    h_distances=scrapy.Field()
    h_raceclasses=scrapy.Field()
    h_draws=scrapy.Field()
    h_ratings= scrapy.Field()
    h_trainers=scrapy.Field()
    h_jockeys=scrapy.Field()
    h_lbws = scrapy.Field()
    h_winodds=scrapy.Field()
    h_rps=scrapy.Field()
    h_finishtimes=scrapy.Field()
    h_horseweights=scrapy.Field()
    h_gears=scrapy.Field()

class HkjcHorseItem(scrapy.Item):
    racenumber = scrapy.Field()
    racedate = scrapy.Field()
    racetime = scrapy.Field()
    racecoursecode = scrapy.Field()
    raceindex = scrapy.Field()
    racename = scrapy.Field()
    horsenumber = scrapy.Field()
    horsename = scrapy.Field()
    horsecode = scrapy.Field()
    timelist = scrapy.Field()
    zero2finish = scrapy.Field()
    sirename = scrapy.Field()
    h_racedate = scrapy.Field()
    h_place = scrapy.Field()
    h_lbw = scrapy.Field()
    h_rp = scrapy.Field()
    h_sp = scrapy.Field()
    h_racelink = scrapy.Field()
    h_rating = scrapy.Field()
    h_distance = scrapy.Field()
    h_ftime = scrapy.Field()
    h_avgspd = scrapy.Field()
    h_avgspd_n = scrapy.Field()
    h_rc_track_course = scrapy.Field()
    h_rc=scrapy.Field()
    h_surface = scrapy.Field()
    h_course = scrapy.Field()
    h_raceclass = scrapy.Field()
    h_avgspd_cd = scrapy.Field()
    h_avgspd_d = scrapy.Field()
    h_avgspd_l3 = scrapy.Field()
    corr_avgspd_place= scrapy.Field()
    final_sec_time = scrapy.Field()
    raceclass = scrapy.Field()
    racedistance = scrapy.Field()
    racegoing = scrapy.Field()
    racetrack = scrapy.Field()
    racesurface = scrapy.Field()
    horsecodelist = scrapy.Field()

   
    draw = scrapy.Field()
    lbw = scrapy.Field()
    runningposition = scrapy.Field()
    finishtime = scrapy.Field()
    winodds = scrapy.Field()
    winoddsrank = scrapy.Field()
    marginsbehindleader = scrapy.Field()
    earlypacepoints = scrapy.Field()

    dam = scrapy.Field()
    damsire = scrapy.Field()
    position= scrapy.Field()
    horsereport = scrapy.Field()
    corr_draw_place = scrapy.Field()
    corr_sp_place = scrapy.Field()
    dayssincelastrun = scrapy.Field()
    h_avgspd_surface = scrapy.Field()
    h_avgspd_surface_n = scrapy.Field()
    h_avgspd_distance= scrapy.Field()
    h_avgspd_distance_n = scrapy.Field()
    h_winningspeeds= scrapy.Field()
    h_winningratings = scrapy.Field()
    h_avgwinninglbw = scrapy.Field()
    place = scrapy.Field()
    h_passedlsec= scrapy.Field()
    corr_sp_place = scrapy.Field()
    corr_ftime_place = scrapy.Field()
    h_avgspd_season  = scrapy.Field()
    h_bestavgspd_season = scrapy.Field()
    runs_this_season=scrapy.Field()
    tce = scrapy.Field()
    f4 = scrapy.Field()
    qtt = scrapy.Field()
    dt = scrapy.Field()
    tt = scrapy.Field()
    ttc = scrapy.Field()
    sixup = scrapy.Field()
    h_passedlsec_L123 = scrapy.Field()
    ranks_avgspeeds = scrapy.Field()
    earlypacepoints_this =scrapy.Field()
    dayssincelastrun= scrapy.Field()
    careerwinpc  = scrapy.Field()